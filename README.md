...
**Phase 1 (geometry) — done.** Two-body propagation, time-of-closest-approach finding, and miss-distance-vs-time analysis are implemented and tested. Phases 2–4 (multi-object screening, probability of collision, maneuver recommendations) are not yet built — see [Roadmap](#roadmap).
...
- [`conjunction_risk/state.py`](conjunction_risk/state.py) — `StateVector`: position/velocity of an object at an epoch, in an inertial (ECI) frame.
